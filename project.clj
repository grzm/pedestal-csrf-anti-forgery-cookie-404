(defproject csrf-tok "0.0.1-SNAPSHOT"
  ...
  :profiles {:dev {:aliases {"run-dev" ["trampoline" "run" "-m" "csrf-tok.server/run-dev"]}
                   :dependencies [[io.pedestal/pedestal.service-tools "0.5.1"]]}
             :uberjar {:aot [csrf-tok.server]}}
  :main ^{:skip-aot true} csrf-tok.server)
